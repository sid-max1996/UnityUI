...
public class MainControl : MonoBehaviour {
    ...
    public void Start()
    {
        ...
        input1.onValidateInput += delegate (string input, int charIndex, char addedChar) { return OpValidate(addedChar); };
        input2.onValidateInput += delegate (string input, int charIndex, char addedChar) { return OpValidate(addedChar); };
        for (int i = 1; i <= 4; i++)
        {
    ...
    private char OpValidate(char c)
    {
        if (c == '+' || c == '-' || c == '*' || c == '/' || c == '=')
        {
            c = '\0';
